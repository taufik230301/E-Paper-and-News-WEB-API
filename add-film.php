...
if (isset($_POST['method']) && $_POST['method']=="Add") {
	...
	$query = "INSERT INTO tbl_film (featured, isBreaking, isTrending, news_title, news_cat, news_description, news_cover_img,published_on, status) VALUES('".trim($_POST['isFeatured'])."','".trim($_POST['isBreaking'])."','".trim($_POST['isTrending'])."','".addslashes(trim($_POST['txtTitle']))."','".$_cid."','".$strDummy."','".$file_name."', now(),'".trim($_POST['isPublish'])."')";
	//echo $query;
	$DB->query($query) or die("Insert Error at line 43: ".$DB->error());
	$_insetId=$DB->insert_id();
	//print_r($_insetId);
	$strCatlist=str_replace('**',$_insetId,$strCatlist);
	$query_catlist="INSERT INTO tbl_film_category (news_id,category_id) VALUES ".substr($strCatlist, 0, -1).";" or die("Category INSERT Error: ".$DB->error());
...
if (isset($_POST['method']) && $_POST['method']=="Edit") {
	...
	$query_clear="DELETE FROM tbl_film_category WHERE news_id=".$_POST['cid'];
	$DB->query($query_clear);
	$query_catlist="INSERT INTO tbl_film_category (news_id,category_id) VALUES ".substr($strCatlist, 0, -1);
	//echo($query_catlist);
	...
	if($_FILES['catImage']['error'] > 0) { 
      $query = "UPDATE tbl_film SET news_title='".addslashes(trim($_POST['txtTitle']))."', news_cat='".$_cid."', featured='".trim($_POST['isFeatured'])."', isBreaking='".trim($_POST['isBreaking'])."', isTrending='".trim($_POST['isTrending'])."', status='".trim($_POST['isPublish'])."', news_description='".$strDummy."' WHERE id=".$_POST['cid'];
	}else{
		$imageFileType = strtolower(pathinfo($_FILES['catImage']['name'],PATHINFO_EXTENSION));
		$file_name = strtolower(str_replace(" ","_",$_FILES['catImage']['name']));
		$file_tmp =$_FILES['catImage']['tmp_name'];
		...
		$query = "UPDATE tbl_film SET news_title='".addslashes(trim($_POST['txtTitle']))."', news_cat='".$_cid."', featured='".trim($_POST['isFeatured'])."', isBreaking='".trim($_POST['isBreaking'])."', isTrending='".trim($_POST['isTrending'])."', status='".trim($_POST['isPublish'])."', news_description='".$strDummy."', news_cover_img='".$file_name."' WHERE id=".$_POST['cid'];	
	}
	//echo $query;die();
...
if (isset($_GET['action']) && $_GET['action']=="edit") {
	$_isRequired="";
	$_method="Edit";
	$query = "SELECT * FROM tbl_film WHERE id ='".trim($_GET['news'])."'";
        ...
        <div class="main-panel">
          <div class="content-wrapper">
              ...
              <nav aria-label="breadcrumb">
                ...
                <div class="card">
                  <div class="card-body">
                    <h4 class="card-title">Masukkan Detail Iklan Film</h4>
               		<?=$message?>
                    <form class="forms-sample" method="post" action="add-film.php" enctype="multipart/form-data">
                      <div class="form-group">
                        <label for="txtTitle"><strong>Judul Film</strong></label>
                        <input type="text" class="form-control" id="txtTitle" name="txtTitle" placeholder="News Title" value="<?=$_newsTitle;?>" required>
                      </div>
					  <div class="form-group">
						<label for="isFeatured"><strong>Apakah Iklan Film ?</strong></label><p class="alert alert-danger"> *If selected News will be placed on <strong>Featured</strong> Section of App Home screen of News App.</p>
                        ...
                        <div class="row" style="margin-left: 5px;">
							<div class="form-check col-1">
                                <label class="form-check-label">
                                  <input type="radio" class="form-check-input" name="isTrending" id="isTrending1" value="1" <?php if($_isTrending=='0'){echo'checked';}?>> Berita </label>
                              </div>
                            
                              <div class="form-check">
                                <label class="form-check-label">
                                  <input type="radio" class="form-check-input" name="isTrending" id="isTrending2" value="0" <?php if($_isTrending=='1'){echo'checked';}?>> Iklan Film </label>
                              </div></div>
